...
zammadClient.interceptors.response.use(
  ...
  (err) => {
    logger.error(
      { status: err.response?.status, url: err.config?.url, data: err.response?.data },
      'Zammad API error',
    );
...
export async function getTicket(ticketId) {
  const { data } = await zammadClient.get(`/tickets/${ticketId}`, { params: { expand: true } });
  return data;
}

export async function getTicketArticles(ticketId) {
  const { data } = await zammadClient.get(`/ticket_articles/by_ticket/${ticketId}`);
  return data;
}

export async function createTicket({ title, body, customerId, groupId, priorityId = 2, customAttributes = {} }) {
  const payload = {
    title,
...
export async function updateTicketState(ticketId, stateId) {
  const { data } = await zammadClient.put(`/tickets/${ticketId}`, { state_id: stateId });
...
export async function updateTicketStatusByName(ticketId, localStatus) {
  ...
  if (!state) {
    const err = new Error(`Zammad state not found: "${stateName}". Create it at /ticket_states.`);
    err.status = 422;
    throw err;
  }
...
export async function assignTicketOwner(ticketId, ownerId) {
  const { data } = await zammadClient.put(`/tickets/${ticketId}`, { owner_id: ownerId });
...
export async function getUserByEmail(email) {
  const { data } = await zammadClient.get('/users/search', { params: { query: email, limit: 1 } });
  return data?.[0] ?? null;
}
...
export async function setupCustomStates() {
  ...
  const pendType  = stateTypes.find((t) => t.name === 'pending reminder' || t.name === 'pending');
  ...
  for (const stateData of toCreate) {
    try {
      const { data: created } = await zammadClient.post('/ticket_states', stateData);
      const key = created.name === 'Em andamento' ? 'em_andamento' : 'aguardando';
      results[key] = { status: 'created', state: { id: created.id, name: created.name } };
    } catch (err) {
      const key = stateData.name === 'Em andamento' ? 'em_andamento' : 'aguardando';
  ...
  for (const name of ['Em andamento', 'Aguardando']) {
    ...
    if (!results[key]) {
      ...
      results[key] = existing
        ? { status: 'already_exists', state: { id: existing.id, name: existing.name } }
...
function pickTreeClassificationAttribute(attributes, preferredName) {
  ...
  return [...candidates].sort((a, b) => (a.position ?? 0) - (b.position ?? 0))[0];
}
...
function buildFieldsClassification(attributes, classificationSteps) {
  const byName = new Map(
    attributes
      .filter((f) => f.object === 'Ticket' && (f.data_type === 'select' || f.data_type === 'multiselect'))
      .map((f) => [f.name, f]),
  );

  const steps = classificationSteps.map((step) => {
    const attr    = byName.get(step.name);
    if (!attr) logger.warn({ name: step.name }, 'Ticket select attribute not found in Zammad');
...
function autoDiscoverClassificationSteps(attributes, groups = []) {
  ...
  for (const attr of selectAttrs) {
    ...
    for (const gk of groupKeys) {
      ...
      if (score > bestScore) { bestScore = score; bestGroup = gk.name; }
    }
  ...
  for (const g of groups) {
    ...
    for (const cat of cats) {
      steps.push({ name: cat.name, label: cat.display || cat.name, when: { group: g }, position: cat.position ?? 0 });
    }
    for (const sub of subs) {
      const when = cats.length === 1 ? { [cats[0].name]: '*' } : { group: g };
      steps.push({ name: sub.name, label: sub.display || sub.name, when, position: sub.position ?? 0 });
    }
  }
...
export async function fetchTicketObjectAttributes() {
  const { data } = await zammadClient.get('/object_manager_attributes', { params: { object: 'Ticket' } });
  return data;
}
 ...
 */
export async function getTicketFields({ treeFieldName, classificationSteps = [] } = {}) {
  let groups = [];
  try {
  ...
  try {
    ...
    if (stepDefs.length > 0) {
      ...
      logger.info({ mode: 'fields', stepCount: classification.steps.length, autoDiscovered: classification.autoDiscovered }, 'Loaded ticket classification (multi-select)');
      return { groups, classification };
    }

    const classification = { mode: 'none', field: null, display: null, tree: [], steps: [] };
    ...
    if (treeAttr) {
      ...
      logger.info({ field: classification.field, depthSample: classification.tree.length }, 'Loaded ticket classification tree');
    }

    return { groups, classification };
  } catch (err) {
    logger.error({ error: err.message, stack: err.stack, status: err.response?.status }, 'Error fetching ticket fields');
    return { groups, classification: { mode: 'none', field: null, display: null, tree: [], steps: [] } };
  }
}
