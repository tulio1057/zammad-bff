...
export async function getTicketAttributes() {
  try {
    const { data } = await zammadClient.get(
      "/object_manager_attributes?object=Ticket",
    );
    return data;
  } catch (err) {
    logger.error({ error: err.message }, "Failed to fetch ticket attributes");
    return [];
  }
}
...
export async function getPriorities() {
  try {
    const { data } = await zammadClient.get("/priorities");
    return data;
  } catch (err) {
    logger.error({ error: err.message }, "Failed to fetch priorities");
    return [];
  }
}
