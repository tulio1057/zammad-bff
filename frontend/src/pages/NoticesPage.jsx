...
import { fetchNotices, createNotice } from '../services/notice.service.js';
...
export default function NoticesPage() {
  ...
  const loadNotices = useCallback(() => {
    ...
    fetchNotices()
      .then(setNotices)
      .catch((err) => setError(err.response?.data?.error || 'Erro ao carregar avisos'))
  ...
  async function handleSubmit(e) {
    ...
    try {
      const newNotice = await createNotice(formData.title || null, formData.message);
      setNotices(prev => [newNotice, ...prev]);
      setFormData({ title: '', message: '' });
      setShowForm(false);
    } catch (err) {
      setError(err.response?.data?.error || err.message || 'Erro ao criar aviso');
    } finally {
      setSubmitting(false);
    }
  }
