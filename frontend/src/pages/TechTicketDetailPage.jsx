...
import { fetchTechTicket, assignTicket, changeStatus, addUpdate } from '../services/technician.service.js';
  ...
  return (
    <div className="layout">
      <main className="main" style={{ marginLeft: 0, maxWidth: '100%' }}>
        <button className="btn btn-ghost back-btn" onClick={() => navigate('/tech')}>← Voltar</button>
            ...
            <div className="tech-detail-left">
              ...
              <div className="tech-actions">
                {ticket.status === 'aberto' && !isAssigned && (
                  <button className="btn btn-primary" style={{ width: 'auto' }} onClick={handleAssign} disabled={submitting}>
                    ✋ Assumir Chamado
                  </button>
                )}
                {canAct && transitions.map(s => (
                  <button key={s} className="btn btn-ghost" onClick={() => handleStatus(s)} disabled={submitting}>
                    → {STATUS_LABELS[s]}
                  </button>
                ))}
              </div>
              ...
              <div className="status-tabs" style={{ marginTop: 24 }}>
                <button className={`status-tab ${activeTab === 'updates' ? 'active' : ''}`} onClick={() => setActiveTab('updates')}>Atualizações</button>
                <button className={`status-tab ${activeTab === 'articles' ? 'active' : ''}`} onClick={() => setActiveTab('articles')}>Histórico Zammad</button>
              </div>

              {activeTab === 'updates' && (
                <div className="updates-list">
                  {data.updates.length === 0 && <p style={{ color: 'var(--gray-600)', fontSize: 14, marginTop: 12 }}>Sem atualizações ainda.</p>}
                  {data.updates.map(u => (
                    <div key={u.id} className="update-item">
                      <div className="update-header">
                        <strong>{u.author_name}</strong>
                        <span>{new Date(u.created_at * 1000).toLocaleString('pt-BR')}</span>
                      </div>
                      {u.status_to && (
                        <span className="update-status-change">
                          {STATUS_LABELS[u.status_from]} → {STATUS_LABELS[u.status_to]}
                        </span>
                      )}
                      <p>{u.message}</p>
                    </div>
                  ...
                  {canAct && (
                    <form onSubmit={handleUpdate} style={{ marginTop: 16 }}>
                      ...
                      <button type="submit" className="btn btn-primary" style={{ width: 'auto' }} disabled={submitting || !updateMsg.trim()}>
